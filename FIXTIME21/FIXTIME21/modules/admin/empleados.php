...
.theme-light .hint { color:#475569; }

/* =====================================================================
   TOASTS
   ===================================================================== */
.toasts{position:fixed; top:18px; right:18px; z-index:1300; display:flex; flex-direction:column; gap:10px}
.toast{ width:min(440px, 92vw); background:#ffffff; color:#111827; border:1px solid rgba(15,23,42,.08); border-radius:14px; box-shadow:0 20px 40px rgba(15,23,42,.12); padding:12px 14px 10px; display:flex; gap:10px; align-items:flex-start; animation:toastIn .22s ease-out both; }
...
/* =====================================================================
   MODAL CONFIRM BAJA
   ===================================================================== */
.confirm-backdrop{ position:fixed; inset:0; z-index:1400; background:rgba(0,0,0,.55); backdrop-filter:blur(2px); display:none; align-items:center; justify-content:center; }
.confirm-backdrop.show{ display:flex; }
.confirm-modal{ width:min(560px,96vw); border-radius:16px; padding:22px; background:#ffffff; color:#0b1220; border:1px solid rgba(15,23,42,.08); box-shadow:0 24px 60px rgba(15,23,42,.18); text-align:center; animation:toastIn .22s ease-out both; }
.confirm-icon{ width:110px; height:110px; border-radius:999px; margin:0 auto 12px; display:grid; place-items:center; border:6px solid #f59e0b20; color:#f59e0b; font-size:56px; font-weight:700; }
...
/* =====================================================================
   MODAL EDICIÓN
   ===================================================================== */
.modal-backdrop{ position:fixed; inset:0; z-index:1400; background:rgba(0,0,0,.55); backdrop-filter:blur(2px); display:none; align-items:center; justify-content:center; }
.modal-backdrop.show{ display:flex; }
.modal { width: min(980px, 96vw); max-height: none; height: auto; overflow: visible; border-radius: 16px; background: var(--panel); color: var(--text); border: 1px solid rgba(157,176,208,18); box-shadow: 0 24px 60px rgba(0,0,0,35); padding: 22px; }
...
.btn-danger:hover { background: #d62828; transform: translateY(-2px); }

/* =====================================================================
   >>> ESTILO PARA SELECTORES SEGMENTADOS <<<
   ===================================================================== */

/* Contenedor principal para ambos selectores */
.segmented-group {
    display: inline-flex;
    flex-wrap: wrap; /* Permite que los cargos se ajusten a varias líneas */
    gap: 4px; /* Espacio entre botones */
    background: var(--panel-2);
    border-radius: 14px; /* Borde del contenedor */
    padding: 4px;
    border: 1px solid rgba(157, 176, 208, .18);
    margin-bottom: 12px;
}
.segmented-group.locked {
    pointer-events: none;
    opacity: .65;
}
...
.segmented-group .segmented-btn {
    ...
    font-weight: 700;
    font-size: 14px;
    border-radius: 10px; /* Botones también redondeados */
...
.segmented-group .segmented-btn:not(.active):hover {
    color: var(--text);
}
      ...
      <?php
        ...
        ob_start(); ?>
          <div>
            ...
            <div class="segmented-group" id="altaMode" role="tablist" aria-label="Modo de alta">
              <button type="button" class="segmented-btn active" data-mode="empleado" role="tab" aria-selected="true">Empleado</button>
              <button type="button" class="segmented-btn" data-mode="admin" role="tab" aria-selected="false">Administrador</button>
            </div>
          </div>

          <div id="cargoWrapper">
            <label>Seleccionar Cargo</label>
            <input type="hidden" name="cargo_id" id="cargoInput" value="">
            <div class="segmented-group" id="cargoContainer" role="group" aria-label="Selección de cargo">
                <?php
                $cargos = db()->query("SELECT id, descripcion FROM Cargos ORDER BY descripcion")->fetchAll(PDO::FETCH_ASSOC);
                $adminCargoId = null;
                foreach($cargos as $c) {
                    $isAdmin = (strcasecmp((string)$c['descripcion'],'Administrador')===0);
                    if ($isAdmin) $adminCargoId = (int)$c['id'];
                    echo '<button type="button" class="segmented-btn" data-cargo-id="' . (int)$c['id'] . '"' . ($isAdmin ? ' data-admin="1"' : '') . '>' . h($c['descripcion']) . '</button>';
                }
                ?>
            </div>
          </div>
...
(function () {
  ...
  // === Modo Alta: Empleado / Administrador y Selección de Cargo (Segmented controls) ===
  (function(){
    const altaModeGroup = document.getElementById('altaMode');
    ...
    if (!altaModeGroup || !cargoContainer || !cargoInput || !cargoWrapper) return;

    const adminCargoBtn = cargoContainer.querySelector('[data-admin="1"]');
    const adminModeBtn = altaModeGroup.querySelector('[data-mode="admin"]');
    const empleadoModeBtn = altaModeGroup.querySelector('[data-mode="empleado"]');
    ...
    cargoContainer.addEventListener('click', (e) => {
        ...
        cargoContainer.querySelectorAll('.segmented-btn').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');

        // Guardar valor en el input hidden
        const cargoId = btn.dataset.cargoId;
        cargoInput.value = cargoId;

        // Sincronizar con el modo de alta
        if (btn.dataset.admin === '1') {
            setAltaMode('admin', false);
        } else {
            setAltaMode('empleado', false);
        }
    });

    // Función para manejar el cambio de modo de alta
    altaModeGroup.addEventListener('click', (e) => {
        const btn = e.target.closest('.segmented-btn');
        if (!btn) return;
        setAltaMode(btn.dataset.mode, true);
    });

    function setAltaMode(mode, triggerCargoClick) {
        // Actualizar botones de modo alta
        [empleadoModeBtn, adminModeBtn].forEach(b => {
        ...
        if (mode === 'admin') {
            ...
            if (!adminCargoBtn) {
                alert('No existe el cargo "Administrador" en Cargos. Agregalo y recargá.');
                return;
            }
            if (triggerCargoClick) adminCargoBtn.click();
        } else { // modo empleado
            cargoWrapper.style.display = 'block'; 
            
            // CORRECCIÓN: Ocultar el botón de admin de la lista de cargos
            if (adminCargoBtn) {
              adminCargoBtn.style.display = 'none';
            }
            
            // Si el cargo activo era el de admin, deseleccionarlo
            const activeCargo = cargoContainer.querySelector('.segmented-btn.active');
            if (activeCargo && activeCargo.dataset.admin === '1') {
                activeCargo.classList.remove('active');
                cargoInput.value = '';
            }
        }
    }

    // Inicializar en modo empleado
    setAltaMode('empleado', false);
  })();
