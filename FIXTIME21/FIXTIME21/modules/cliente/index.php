...
$esEmpleado = $repoE->esEmpleado((int)($_SESSION['uid'] ?? 0));
...
try {
    ...
    $st = $db->prepare("SELECT 1 FROM Empleados WHERE Persona_id=? AND (fecha_baja IS NULL OR fecha_baja > CURDATE()) LIMIT 1");
    $st->execute([$uid]);
    $esEmpleado = (bool)$st->fetchColumn();
...
function redirect_with_flash(string $msg, string $type='success', string $tab='home'): void {
  ...
  header('Location: '.$b.'/modules/cliente/?tab='.urlencode($tab)); exit;
}
...
function empresa_actualizar(int $empresaId, string $razon, string $cuit, ?string $email, ?string $tel): void {
  ...
  foreach ($tipos as $t) {
    ...
    $st->execute([$t]);
    if(!$st->fetchColumn()){
      $i=$db->prepare("INSERT INTO Tipos_Contactos (descripcion) VALUES (?)"); $i->execute([$t]);
    }
  }
    ...
    if ($act === 'update_vehiculo') {
      ...
      try {
        $repoV->actualizar($autoId,$uid,[
          'marca'=>$marca,'modelo'=>$modelo,'anio'=>$anio,'km'=>$km,'color'=>$color,
          'patente'=>$pat,'descripcion_extra'=>$dex,'cedula_frente'=>$fr,'cedula_trasera'=>$dr
        ]);
      } catch (Throwable $e) {
        $code = $e->getMessage();
...
if ($act === 'add_turno') {
    ...
    if (!$autoId) {
        redirect_with_flash('Elegí vehículo.', 'error', 'turnos');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $estado_t = strtolower(trim($_POST['estadoEstado_Turno_id'] ?? ''));

    if ($action === 'reprogramar_turno' && $turnoId > 0) {
        // ID del estado "pendiente reasignacion"
      if ($estado_t === 'cancelado') {
        // ya estaba cancelado → mensaje de error
        redirect_with_flash('No se pudo realizar la acción: el turno esta cancelado.', 'error', 'turnos');
        exit;
    }
      if ($estado_t === 'pendiente reasignacion') {
        // ya estaba cancelado → mensaje de error
        redirect_with_flash('Ya se realizó la acción. Por favor espere que se le reagende el turno.', 'error', 'turnos');
        exit;
    }
        $repoT->reasignar($uid, $turnoId);

        redirect_with_flash('Turno marcado como pendiente de reasignación.', 'success', 'turnos');
    }

    if ($action === 'cancelar_turno' && $turnoId > 0) {
    if ($estado_t === 'cancelado') {
        // ya estaba cancelado → mensaje de error
        redirect_with_flash('Ya se realizó la acción: el turno ya estaba cancelado.', 'error', 'turnos');
        exit;
    }

    // si no estaba cancelado → lo cancelo ahora
...
$tels = $repoP->listarTelefonos($uid); if (!$tels) $tels=[''];
...
<title>Fixtime — Panel de Cliente</title>
...
.col-12{grid-column:span 12}.col-6{grid-column:span 6}.col-4{grid-column:span 4}.col-3{grid-column:span 3}.col-8{grid-column:span 8}.col-5{grid-column:span 5}.col-7{grid-column:span 7}.col-9{grid-column:span 9}.col-2{grid-column:span 2}.col-10{grid-column:span 10}.col-11{grid-column:span 11}
...
.thumb,.thumb-sm{display:inline-block;width:52px;height:52px;border-radius:10px;background:#0b162b center/cover no-repeat;border:1px solid rgba(157,176,208,.25);position:relative;cursor:zoom-in}
.thumb::after,.thumb-sm::after{content:"";position:absolute;left:50%;bottom:100%;transform:translate(-50%,-10px) scale(.95);width:240px;height:180px;border-radius:12px;background:inherit;background-size:contain;background-repeat:no-repeat;background-position:center;box-shadow:0 20px 40px rgba(2,6,23,.45);border:1px solid rgba(157,176,208,.25);opacity:0;pointer-events:none;transition:opacity .12s ease, transform .12s ease;z-index:20}
.thumb:hover::after,.thumb-sm:hover::after{opacity:1;transform:translate(-50%,-12px) scale(1)}
.table td{overflow:visible}
dialog#vehiculoModal{position:fixed; inset:50% auto auto 50%; transform:translate(-50%,-50%); z-index:1000; border:0; border-radius:18px; padding:0; max-width:880px; width:95%; background:var(--panel); box-shadow:0 30px 80px rgba(2,6,23,.60)}
dialog#vehiculoModal::backdrop{background:rgba(5,10,25,.55);backdrop-filter:blur(4px) saturate(120%)}
body.modal-open{overflow:hidden}
body.modal-open .thumb::after, body.modal-open .thumb-sm::after{display:none!important}
...
@media (max-width:1080px){
  ...
  .col-6{grid-column:span 6}.col-4{grid-column:span 6}.col-3{grid-column:span 3}.col-5{grid-column:span 6}.col-7{grid-column:span 6}.col-9{grid-column:span 6}.col-10{grid-column:span 6}.col-11{grid-column:span 6}
}
.table.responsive { width: 100%; }
@media (max-width: 740px) {
  .table.responsive thead { display: none; }
  .table.responsive tbody tr { display: block; background: linear-gradient(180deg, rgba(255,255,255,.05), rgba(255,255,255,.03)); border: 1px solid rgba(157,176,208,.16); border-radius: 14px; padding: 12px; margin-bottom: 12px; box-shadow: 0 10px 24px rgba(2,6,23,.25); }
  .table.responsive tbody td { display: grid; grid-template-columns: 120px 1fr; gap: 8px; padding: 8px 4px; border: 0; }
  .table.responsive tbody td::before { content: attr(data-label); color: var(--muted); font-size: 12px; font-weight: 700; }
  .table.responsive .thumb-sm { width: 46px; height: 46px; border-radius: 10px; }
  .veh-acciones { display: flex !important; gap: 10px; justify-content: flex-end; margin-top: 4px; }
}
.table.responsive th, .table.responsive td { vertical-align: middle; }
.input-patente{min-width:220px}
@media (min-width: 740px) { #vehiculoModal form .patente-wide { grid-column: span 6; } }
.banner-empresa{background:linear-gradient(135deg, rgba(59,130,246,.18), rgba(37,99,235,.14)); border:1px solid rgba(59,130,246,.35); padding:12px 14px; border-radius:12px; margin-bottom:10px; font-size:14px}
/* Toggle estilizado */
.toggle{display:flex; border:1px solid rgba(157,176,208,.3); border-radius:12px; overflow:hidden; width:max-content}
.toggle input{display:none}
.toggle label{padding:10px 14px; cursor:pointer; user-select:none; font-weight:800}
.toggle input:checked + label{background:linear-gradient(135deg,var(--brand),var(--brand-2)); color:#0b1220}
</style>
</head>
<body>
...
<div class="app">
  ...
  <?php if ($esAdmin): ?>
    <a href="<?= $base ?>/modules/selector/index.php" class="btn ghost topbar-salir">
  ...
  <main class="main">
    ...
    <section id="tab-vehiculos" style="display:none">
      ...
      <div class="card">
        ...
        <table class="table responsive">
          ...
          <tbody>
            <?php if (!$vehiculos): ?>
              <tr><td colspan="10">Aún no registraste vehículos.</td></tr>
                    ...
                    <td style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
                      <form method="post" class="validate-form" style="display:flex;gap:6px;align-items:center">
                        ...
                        <form method="post" class="form-acciones">
                          <input type="hidden" name="csrf" value="<?= h($csrf) ?>">
                          <input type="hidden" name="turno_id" value="<?= (int)$t['id'] ?>">
                          <input type="hidden" name="estadoEstado_Turno_id" value="<?= strtolower($t['estado']) ?>">
                          <form method="post" class="form-eliminar" style="display:inline">
                            <input type="hidden" name="action" value="cancelar_turno">
                            <input type="hidden" name="turno_id" value="<?= (int)$t['id'] ?>">
                            <button type="button" class="btn ghost btn-eliminar">Cancelar</button>
                          
                          <!-- <button class="btn ghost" name="action" value="cancelar_turno">Cancelar</button> -->
                          <!-- <button class="btn ghost" name="action" value="reprogramar_turno">Reprogramar</button> -->
                          <form method="post" class="form-eliminar" style="display:inline">
                            <input type="hidden" name="action" value="reprogramar_turno">
                            <input type="hidden" name="turno_id" value="<?= (int)$t['id'] ?>">
                            <button type="button" class="btn ghost btn-eliminar">Reprogramar</button>
                          <!-- </form> -->
                      </form>
    ...
    <section id="tab-historial" style="display:none">
      <div class="card">
        ...
        <table class="table">
          <thead><tr><th>Fecha ingreso</th><th>Trabajo</th><th>Estado</th><th>Vehículo</th></tr></thead>
          <tbody>
          <?php if (!$historial): ?><tr><td colspan="5">Aún no hay trabajos registrados.</td></tr>
          <?php else: foreach ($historial as $hrow): ?>
            <tr><td><?= h($hrow['fecha_ingreso']) ?></td><td><?= h($hrow['trabajo']) ?></td><td><?= h($hrow['estado']) ?></td><td><?= h($hrow['marca'].' '.$hrow['modelo'].' '.$hrow['anio']) ?></td></tr>
          <?php endforeach; endif; ?>
          </tbody>
        </table>
      </div>
    ...
    <section id="tab-perfil" style="display:none">
      <div class="card">
        <h3>Mis datos</h3>
        <!-- Opción A: el perfil va a modules/cliente/perfil/guardar.php -->
        <form method="post"
              action="<?= $base ?>/modules/cliente/perfil/guardar.php"
              class="row validate-form">
          <input type="hidden" name="csrf" value="<?= h($csrf) ?>">

          <div class="col-6"><label>Nombre</label>
            <input name="nombre" value="<?= h($nom) ?>" required>
          </div>
          <div class="col-6"><label>Apellido</label>
            <input name="apellido" value="<?= h($ape) ?>" required>
          </div>

          <div class="col-6"><label>DNI</label>
            <input name="dni" value="<?= h($dniAct) ?>" required>
          </div>
          <div class="col-6"><label>Email</label>
            <input type="email" name="email" value="<?= h($emailAct) ?>">
          </div>
          ...
          <div style="padding:12px 0;border-bottom:1px solid rgba(157,176,208,.14)">
            ...
            <?php if (!empty($e['contactos'])): ?>
              <table class="table" style="margin-top:10px">
                <thead><tr><th>Tipo</th><th>Valor</th><th style="width:140px">Acciones</th></tr></thead>
                <tbody>
...
<?php if ($flash): ?>
Swal.fire({toast:true,position:'top-end',icon:'<?= $flashType ?>',title:'<?= h($flash) ?>',showConfirmButton:false,timer:3200,timerProgressBar:true});
<?php endif; ?>
...
(function(){
  const f=document.getElementById('formAlta'); if(!f) return;
  const MAX=5*1024*1024, ok=['image/jpeg','image/png','image/webp'];
  f.addEventListener('submit',e=>{
    ...
    if (tipo === 'empresarial') {
      ...
      if (!emp) { e.preventDefault(); Swal.fire({icon:'warning',title:'Elegí una empresa',text:'Vinculá una empresa en el panel Empresa y luego volvé a este formulario.'}); return; }
    }
    for(const n of ['cedula_frente','cedula_dorso']){
      const file=f[n].files[0];
      if(!file){ e.preventDefault(); Swal.fire({icon:'error',title:'Faltan las fotos de cédula'}); return; }
      if(file.size>MAX){ e.preventDefault(); Swal.fire({icon:'error',title:'Archivo muy grande (máx 5MB)'}); return; }
      if(!ok.includes(file.type)){ e.preventDefault(); Swal.fire({icon:'error',title:'Formato inválido (JPG/PNG/WEBP)'}); return; }
    }
  });
...
document.querySelectorAll('.form-cancelar').forEach(f=>f.addEventListener('submit',e=>{
  e.preventDefault();
  Swal.fire({title:"¿Cancelar turno?",icon:"warning",showCancelButton:true,confirmButtonText:"Sí, cancelar",cancelButtonText:"No",confirmButtonColor:"#ef4444"}).then(r=>{if(r.isConfirmed)f.submit();});
}));
document.querySelectorAll('.form-delete').forEach(f=>f.addEventListener('submit',e=>{
  e.preventDefault();
  Swal.fire({title:"¿Eliminar vehículo?",text:"Si no tiene vínculos, se borrará definitivamente.",icon:"warning",showCancelButton:true,confirmButtonText:"Sí, eliminar",cancelButtonText:"No",confirmButtonColor:"#ef4444"}).then(r=>{if(r.isConfirmed)f.submit();});
}));
const modal=document.getElementById('vehiculoModal'); const mClose=document.getElementById('m_close');
function openVehiculoModal(){ if(typeof modal.showModal==='function') modal.showModal(); else modal.setAttribute('open',''); document.body.classList.add('modal-open'); }
function closeVehiculoModal(){ if(typeof modal.close==='function') modal.close(); else modal.removeAttribute('open'); document.body.classList.remove('modal-open'); }
document.querySelectorAll('.btn-edit').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    const fr=btn.dataset.foto_fr||''; const dr=btn.dataset.foto_dr||'';
...
modal.addEventListener('cancel',e=>{e.preventDefault();closeVehiculoModal();});
...
document.getElementById('telefonos-wrap')?.addEventListener('click',e=>{
  const b=e.target.closest('.btn-del-tel'); if(!b) return;
  const item=b.closest('.tel-item'); if(!item) return;
  const total=item.parentElement.querySelectorAll('.tel-item').length;
  if(total>1) item.remove(); else item.querySelector('input').value='';
});
(function(){
...
<!-- Footer del cliente igual al Recepcionista (sin cambiar tipografías globales) -->
<style>
/* Footer fijo */
.sidebar .sidebar-footer{margin-top:auto;display:grid;gap:1rem;padding-top:1rem;}
/* Botones ghost idénticos a admin: centrados, sin animación, borde tenue */
.sidebar .sidebar-footer .btn.ghost{
  justify-content:center; /* centrado horizontal, como admin */
  background:transparent; color:var(--text);
  border:1px solid rgba(157,176,208,.30); box-shadow:none; transition:none;
  /* Tipografía igual a los ítems del menú */
  font-family:inherit; font-weight:600; font-size:1.05rem; line-height:1.1; letter-spacing:0;
}

.sidebar .sidebar-footer .btn.ghost:hover{background:transparent; color:var(--text); border-color:rgba(157,176,208,.30);}

/* Ajuste de icono del botón 'volver' para que no desbalancee el centrado */
.sidebar .sidebar-footer .btn.ghost .icon, 
.sidebar .sidebar-footer .btn.ghost svg{margin-right:.4rem;}

/* Asegurar igual alto y radio */
.sidebar .sidebar-footer .btn{padding:1rem 1.1rem;border-radius:var(--radius,24px);}
</style>
