...
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
    header("Location: $base/modules/admin/empleados.php?err=" . urlencode("ID inválido"));
    exit;
}

try {
    $repoE->desactivar($id); // ahora hace UPDATE fecha_baja
    header("Location: $base/modules/admin/empleados.php?ok=" . urlencode("Empleado dado de baja"));
} catch (Throwable $e) {
