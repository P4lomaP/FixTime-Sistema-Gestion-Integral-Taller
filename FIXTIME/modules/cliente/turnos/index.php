...
<main class="page-center">
  <div class="card">
    <h2>Mis turnos</h2>
    <?php if ($ok): ?><div class="msg-ok">Operación exitosa.</div><?php endif; ?>
    ...
    <?php else: ?>
      <div class="tabla">
        <div class="tabla-row tabla-head">
          <div>Fecha</div><div>Hora</div><div>Vehículo</div><div>Estado</div><div>Acciones</div>
        </div>
        <?php foreach ($turnos as $t): ?>
          <div class="tabla-row">
            <div><?= htmlspecialchars($t['fecha_turno']) ?></div>
            <div><?= htmlspecialchars(substr($t['hora_turno'],0,5)) ?></div>
            ...
            <div>
              ...
              <form action="solicitar.php" method="get" style="display:inline">
                <input type="hidden" name="reprogramar" value="<?= (int)$t['id'] ?>">
                <button class="btn secundario">Reprogramar</button>
              </form>
            </div>
          </div>
        <?php endforeach; ?>
