  ...
  try {
    if ($act==='add_vehiculo') {
      ...
      if (!$marcaId || $modelo==='' || $anio==='') {
        $flash = 'Completá marca, modelo y año.';
      } else {
    ...
    if ($act==='add_turno') {
      ...
      if (!$autoId || $fecha==='' || $hora==='') {
        $flash = 'Elegí vehículo, fecha y hora.';
      } else {
        if (!$repoV->perteneceAPersona($uid, $autoId)) {
          $flash = 'El vehículo no te pertenece.';
        } elseif (!$repoT->disponible($fecha,$hora)) {
          $flash = 'No hay disponibilidad para esa fecha/hora.';
        } else {
          $repoT->crear($uid,$autoId,$fecha,$hora);
          $flash = 'Turno solicitado (Pendiente).';
        }
      }
    }
    ...
    if ($act==='reprogramar_turno') {
      ...
      if ($turnoId && $autoId && $fecha && $hora) {
        if (!$repoV->perteneceAPersona($uid,$autoId)) {
          ...
          $flash='Turno reprogramado.';
        }
      } else $flash='Completá los campos.';
    }

    if ($act==='cancelar_turno') {
      $turnoId = (int)($_POST['turno_id'] ?? 0);
      if ($turnoId) { $repoT->cancelar($uid,$turnoId); $flash='Turno cancelado.'; }
    }
    ...
    if ($act==='update_perfil') {
      ...
      if ($n==='' || $a==='' || $dni==='') {
        $flash = 'Completá nombre, apellido y DNI.';
      } elseif ($em!=='' && !filter_var($em,FILTER_VALIDATE_EMAIL)) {
        $flash = 'Email inválido.';
      } else {
        $repoP->actualizarPerfil($uid,$n,$a,$dni,$em);
        // refrescar sesión y variables para ver el cambio YA
    ...
    $flash = 'Ocurrió un error. Intentá nuevamente.';
  }
}
...
.sidebar{
  ...
  padding:22px;background:linear-gradient(180deg,var(--panel),var(--bg));
  border-right:1px solid rgba(157,176,208,.15);
}
.brand{display:flex;gap:12px;align-items:center;margin-bottom:22px}
.brand-badge{
  width:48px;height:48px;border-radius:14px;display:grid;place-items:center;
  background:radial-gradient(40px 30px at 30% 20%, rgba(255,255,255,.25), transparent 40%),
...
.theme-btn{
  margin-left:auto;appearance:none;border:1px solid rgba(157,176,208,.28);
  background:rgba(255,255,255,.06); color:var(--text);
  border-radius:12px; padding:10px 12px; cursor:pointer; font-size:16px;
...
.nav button{
  ...
  padding:14px 16px;border-radius:14px;border:1px solid rgba(157,176,208,.18);
  background:rgba(255,255,255,.03);color:var(--text);cursor:pointer;
  font-size:16px;font-weight:700;box-shadow:0 6px 18px rgba(0,0,0,.25);
}
.nav button.active{
  background:linear-gradient(135deg, rgba(59,130,246,.20), rgba(37,99,235,.20));
  border-color:rgba(59,130,246,.55);box-shadow:0 10px 28px var(--ring);
}
.topbar-salir{display:block;margin-top:14px;text-align:center;text-decoration:none}

/* ===== main ===== */
.main{padding:26px 32px}
.flash{margin:0 0 16px;background:rgba(59,130,246,.12);border:1px solid rgba(59,130,246,.40);padding:10px 12px;border-radius:12px}
.hero{
  display:flex;align-items:center;gap:16px;
  background:linear-gradient(135deg, rgba(59,130,246,.22), rgba(37,99,235,.18));
  border:1px solid rgba(59,130,246,.40);
  border-radius:var(--radius); padding:18px;
  box-shadow:0 14px 32px var(--ring); margin-bottom:16px;
}
.hero .avatar{
  width:56px;height:56px;border-radius:14px;display:grid;place-items:center;
  background:radial-gradient(24px 16px at 30% 20%, rgba(255,255,255,.25), transparent 40%),
             linear-gradient(135deg,var(--brand),var(--brand-2));
}
...
.card{background:linear-gradient(180deg,rgba(255,255,255,.05),rgba(255,255,255,.03));
  border:1px solid rgba(157,176,208,.16);border-radius:var(--radius);padding:18px;box-shadow:var(--shadow)}
...
/* ===== tema claro ===== */
.theme-light{
  --bg:#f3f6fc; --panel:#ffffff; --panel-2:#f7f9ff; --card:#ffffff;
  --muted:#5b6b85; --text:#0b1220; --ring:rgba(59,130,246,.28);
  --shadow:0 8px 26px rgba(15,23,42,.08);
}
.theme-light body{
  background:
   radial-gradient(1000px 500px at 80% -10%, rgba(59,130,246,.12), transparent 70%),
   radial-gradient(700px 380px at 10% 110%, rgba(37,99,235,.10), transparent 60%),
   var(--bg);
}
  ...
  <main class="main">
    ...
    <section id="tab-vehiculos" style="display:none">
      <div class="card" style="margin-bottom:16px">
        <h3 style="margin:0 0 10px 0">Registrar vehículo</h3>
          ...
          <div class="col-4">
            ...
            <select name="marca_id" required>
              ...
              <?php foreach ($marcas as $m): ?>
                <option value="<?= (int)$m['id'] ?>"><?= h($m['descripcion']) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
      ...
      <div class="card">
        <h3 style="margin:0 0 10px 0">Mis vehículos</h3>
        <table class="table">
          <thead><tr><th>Marca</th><th>Modelo</th><th>Año</th><th>Color</th><th>KM</th></tr></thead>
          <tbody>
            <?php if (!$vehiculos): ?>
              <tr><td colspan="5" style="opacity:.7">Aún no registraste vehículos.</td></tr>
    ...
    <section id="tab-turnos" style="display:none">
      <div class="card" style="margin-bottom:16px">
        <h3 style="margin:0 0 10px 0">Agendar turno</h3>
        <form method="post" class="row">
          ...
          <div class="col-6">
            ...
            <select name="auto_id" required>
              ...
              <?php foreach ($vehiculos as $v): ?>
                <option value="<?= (int)$v['id_auto'] ?>"><?= h($v['marca'].' '.$v['modelo'].' ('.$v['anio'].')') ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="col-12" style="display:flex;gap:10px"><button class="btn">Solicitar</button></div>
        </form>
      </div>

      <div class="card">
        <h3 style="margin:0 0 10px 0">Mis turnos</h3>
        <table class="table">
          <thead><tr><th>#</th><th>Vehículo</th><th>Fecha</th><th>Hora</th><th>Estado</th><th>Acciones</th></tr></thead>
          <tbody>
          <?php if (!$turnos): ?>
            <tr><td colspan="6" style="opacity:.7">Sin turnos.</td></tr>
            ...
            <tr>
              ...
              <td style="display:flex;gap:8px;flex-wrap:wrap;align-items:center">
                <form method="post" style="display:flex;gap:6px;align-items:center">
                  ...
                  <select name="auto_id" style="padding:6px 8px">
                    <?php foreach ($vehiculos as $v): ?><option value="<?= (int)$v['id_auto'] ?>"><?= h($v['marca'].' '.$v['modelo']) ?></option><?php endforeach; ?>
                  </select>
                  <input type="date" name="fecha" style="padding:6px 8px">
                  <input type="time" name="hora"  style="padding:6px 8px">
    ...
    <section id="tab-historial" style="display:none">
      <div class="card">
        <h3 style="margin:0 0 10px 0">Historial de servicios</h3>
        <table class="table">
          <thead><tr><th>#</th><th>Fecha ingreso</th><th>Trabajo</th><th>Estado</th><th>Vehículo</th></tr></thead>
          <tbody>
          <?php if (!$historial): ?>
            <tr><td colspan="5" style="opacity:.7">Aún no hay trabajos registrados.</td></tr>
    ...
    <section id="tab-perfil" style="display:none">
      <div class="card">
        <h3 style="margin:0 0 10px 0">Mis datos</h3>
        <form method="post" class="row">
          ...
          <div class="col-12" style="display:flex;gap:10px"><button class="btn">Guardar cambios</button></div>
        </form>
      </div>
    </section>
  </main>
...
function applyTheme(t){
  if(t==='light'){ root.classList.add('theme-light'); setIcon('☀️'); }
  else{ root.classList.remove('theme-light'); setIcon('🌙'); }
...
btnThemeMobile && btnThemeMobile.addEventListener('click', toggleTheme);
</script>
</body>
</html>
